...
静态课程网站。首页使用淡蓝色底色、开放式标题布局、分组课程目录与深色交互实验入口；阅读页采用章节导航、正文与页内目录布局，注记和定理使用浅蓝色区块。代码编辑区使用深色界面，运算结果保持浅底展示。支持桌面与移动端，保留东北大学原始校标。所有计算在访客的浏览器执行；不需要后端 Python 服务或 API 密钥。

## 内容来源

- 正式正文：`../docs/textbook/`，10 章、4 附录、前言和全书结构；下载版教材为 192 页。
- 19 幅图使用原始 TikZ 导出 SVG；表格、公式、练习与引用从 TeX 转换。图形在正文中铺满可用宽度；手机端可横向滑动或放大查看。
- 十讲共 312 页原生 HTML 授课幻灯片，包含问题定义、概念阐释、公式推导、分步算例、企业案例和结论；每页链接到讲义中对应小节，可独立授课。支持左右翻页、触屏滑动、页码跳转与全屏。15 个企业案例增设图解与资料边界。
- 17 个交互实验提供背景、任务、输入、输出、约束、示例及评价要点；实验代码、教材片段和源程序提供教学注释。
- 第 11–14 单元、课程大纲、作业、综合项目、报告模板及行业案例。
- 第 12、13 单元的原文件仅含空白提纲，网页将其整理为已有章节与实验的专题阅读入口。
- `../src/bdm_decision/` 的 43 个源文件，包括 30 个已实现 Python 模块、2 个已实现 SQL 脚本和 11 个练习模板（含 YAML 配置）。模板不代表已实现算法。
- 13 份课程 CSV，数据与源码下载包。
...
验证覆盖内部内容链接、19 幅图、超过 1,200 个数学表达式、原有 Python 实验和已实现 Python 模块，以及阈值敏感性、置信区间宽度和优化约束。新增实验、SQL、课件交互和移动端配图需在浏览器中验证。
...
数智化企业运营与优化微专业。教学内容采用 CC BY 4.0；原始下载资料保留其来源与授权信息。网页设计参考 QuantEcon 的阅读结构、香蕉空间的数学内容组织，以及智谱、Claude、Moonshot 的留白、层级与光感；未复制其品牌素材。第三方运行组件采用各自许可证，随网站保存于 `dist/vendor/licenses/`。
