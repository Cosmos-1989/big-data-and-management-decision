...
let teachingSpecsPromise;
function teachingSpecs(){return teachingSpecsPromise ||= fetch('./content/lab-specs.json').then(r=>{if(!r.ok)throw new Error('实验题面加载失败');return r.json();});}
function challengeHtml(spec){if(!spec)return '';const fields=[['任务描述',spec.problem],['输入与参数',spec.input],['预期输出',spec.output],['约束与口径',spec.constraints]];return `<section class="challenge-card" aria-label="实验题面"><div class="challenge-heading"><span>实验题目</span><strong>${esc(spec.title)}</strong></div><div class="challenge-background"><span>案例背景</span><p>${esc(spec.background)}</p></div><div class="challenge-grid">${fields.map(([name,value])=>`<div><h2>${name}</h2><p>${esc(value)}</p></div>`).join('')}</div><div class="challenge-bottom"><div><span>示例与观察</span><p>${esc(spec.example)}</p></div><div><span>评价要点</span><p>${esc(spec.evaluation)}</p></div></div></section>`;}
...
async function runSql(code,gen){
 if(!db){
  status.textContent='正在准备 SQL，请稍候…';
...
export async function mountLab(container,catalog,id='kpi'){
 cleanupLab();const specs=await teachingSpecs();files ||= await(await fetch('./data/files.json')).json();
 ...
 current=source?{...source,chapter:experiments.find(e=>e.id===sourceDemo[source.id])?.chapter,title:source.path,params:[],description:source.status==='template'?'练习文件，请补充代码后运行。':'修改代码后，运行配套示例。'}:snippet?{...snippet,params:[],description:'修改代码，观察计算结果。'}:experiments.find(e=>e.id===id)||experiments[2];
 const spec=specs.experiments.find(item=>item.id===(source?sourceDemo[source.id]:current.id));
 const programGuide=source?specs.programs.find(item=>item.path===source.path):null;
 const snippetGuide=snippet?specs.snippets.find(item=>item.id===snippet.id):null;
 current={...current,code:source?(programGuide?.comment?programGuide.comment+'\n'+current.code:current.code):snippet?(snippetGuide?.annotatedCode||current.code):(spec?.annotatedCode||current.code)};
 const guideHtml=mode==='source'&&programGuide?`<section class="program-guide"><strong>程序导读</strong><p>${esc(programGuide.comment.replace(/^(#|--)\s*/,''))}</p></section>`:mode==='snippet'&&snippetGuide?`<section class="program-guide"><strong>代码注释</strong><p>${esc(snippetGuide.comment.replace(/^(#|--)\s*/,''))}</p></section>`:'';
 container.innerHTML=`<div class="breadcrumb">课程实践 / 交互实验室</div><div class="lab-heading"><div><h1>交互实验室</h1></div><a class="small-link" href="${current.chapter?'#/chapter/'+esc(current.chapter):'#/resources'}">${current.chapter?'返回相关讲义':'返回课程资源'} ↗</a></div><p class="lab-intro">修改参数与代码，探索数据和决策之间的关系。</p><div class="lab-selector"><label for="experiment">选择实验或程序</label><select id="experiment"><optgroup label="课程交互实验">${experiments.map(e=>`<option value="${e.id}">${esc(e.title)}</option>`).join('')}</optgroup><optgroup label="课程程序">${catalog.programs.map(p=>`<option value="source-${p.id}">${esc(p.path.replace('src/bdm_decision/',''))}${p.language==='yaml'?' · 配置':p.status==='template'?' · 练习':''}</option>`).join('')}</optgroup><optgroup label="教材中的 Python / SQL 片段">${catalog.snippets.filter(s=>['python','sql'].includes(s.language)).map(s=>`<option value="${s.id}">${esc(s.title)}</option>`).join('')}</optgroup></select></div><p class="experiment-description">${esc(current.description)}</p>${challengeHtml(spec)}${guideHtml}${current.params.length?`<div class="parameters">${current.params.map(p=>p.type==='bool'?`<label class="bool-param"><input type="checkbox" data-param="${p.name}" ${p.value?'checked':''}>${p.label}</label>`:`<label>${p.label}<output>${p.value}</output><input aria-label="${p.label}" type="range" data-param="${p.name}" min="${p.min}" max="${p.max}" step="${p.step}" value="${p.value}"></label>`).join('')}</div>`:''}<div class="workspace"><section class="code-pane"><div class="pane-heading"><span>${esc(current.language==='sql'?'SQL':current.language==='yaml'?'YAML 配置':'Python')}</span><div><button id="reset-code" class="text-button">恢复示例</button><button id="download-code" class="text-button">下载代码</button></div></div><textarea id="code-editor" aria-label="程序代码编辑器" spellcheck="false" autocapitalize="off" autocomplete="off"></textarea><div class="run-toolbar"><button id="run-code" class="primary">▶ 运行${mode==='source'&&sourceDemo[program?.id]?'配套示例':'代码'}</button><button id="stop-run" class="secondary" disabled>停止</button><span class="keyboard-hint">⌘ / Ctrl + Enter</span></div><details class="runtime-help"><summary>运行说明与数据表</summary><p>可用 <code>display(rows, '标题', x='列名', y='数值列')</code> 生成表格和图形。点击“运行”计算结果，或点击“停止”结束计算。</p><p>SQL 可查询以下数据表：</p><code>${catalog.datasets.map(d=>d.name.replace('.csv','')).join(', ')}</code><p>SQL 脚本包含多个查询时显示最后一个结果；可选中所需查询复制到编辑器单独执行。</p>${mode==='source'&&sourceDemo[program?.id]?`<p>本文件通过“${esc(experiments.find(e=>e.id===sourceDemo[program.id]).title)}”示例调用。</p>`:''}</details></section><section class="output-pane"><div class="pane-heading"><span>运行结果</span><button id="export-result" class="text-button" disabled>下载结果</button></div><div id="run-status" class="run-status" role="status" aria-live="polite">${current.language==='yaml'?'配置文件可编辑和下载，不作为程序执行。':'准备就绪，点击运行开始计算。'}</div><div id="result-output"><div class="waiting-result"><div class="result-symbol">▥</div><h3>在这里观察结果</h3><p>运行后展示表格、图形与程序输出。<br>修改参数，再次运行即可比较。</p></div></div></section></div><p class="lab-footnote">实验使用课程样例数据。</p>`;
