...
import {lectureSlidesHtml,initLectureSlides} from './slides.js';
...
async function chapterPage(id,version){
 ...
 setToc([...(deck?[{title:'授课 Slides',href:`#/chapter/${id}?anchor=slides`}]:[]),...(d.toc||[]).filter(x=>x.level===2).map(s=>({title:s.title,href:`#/chapter/${id}?anchor=${encodeURIComponent(s.id)}`}))]);
}
