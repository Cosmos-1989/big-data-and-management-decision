...
async function chapterPage(id,version){
 const [d,decorations]=await Promise.all([doc(id),chapterDecorations()]);if(version!==renderVersion)return;
 ...
 for(const pre of main.querySelectorAll('.prose pre')){
 ...
 setToc([...(deck?[{title:'授课幻灯片',href:`#/chapter/${id}?anchor=slides`}]:[]),...(d.toc||[]).filter(x=>x.level===2).map(s=>({title:s.title,href:`#/chapter/${id}?anchor=${encodeURIComponent(s.id)}`}))]);
}
